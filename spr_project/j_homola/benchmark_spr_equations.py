...
def beta(eps_m, eps_d, k0):
    return k0*np.sqrt(eps_m*eps_d/(eps_m + eps_d)) 
...
def e_x(x, eps_m, eps_d, k0):
    ...
    for i in range(len(x)):
        if x[i] <= 0:
            e_x[i] = (beta_extended(eps_m, eps_d, k0)/(omega0*eps_m*eps_0))*np.exp(gamma_m(k0, eps_m, eps_d)*x[i])
        else:
            e_x[i] = (beta_extended(eps_m, eps_d, k0)/(omega0*eps_d*eps_0))*np.exp(-gamma_d(k0, eps_m, eps_d)*x[i])
...
def e_z(x, eps_m, eps_d, k0):
    ...
    for i in range(len(x)):
        if x[i] <= 0:
            e_z[i] = (beta_extended(eps_m, eps_d, k0)/(omega0*eps_m*eps_0))*np.exp(gamma_m(k0, eps_m, eps_d)*x[i])
        else:
            e_z[i] = -(beta_extended(eps_m, eps_d, k0)/(omega0*eps_d*eps_0))*np.exp(-gamma_d(k0, eps_m, eps_d)*x[i])
